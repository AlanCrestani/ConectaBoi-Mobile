...
class PerformanceMonitor {
  ...
  void checkPerformanceTargets() {
    ...
    if (incrementalStats['avg'] > 500) {
      alerts.add('Incremental sync averaging ${incrementalStats['avg']}ms (target: <500ms)');
    }
  ...
  String generatePerformanceReport() {
    ...
    if (fullSync['count'] > 0) {
      buffer.writeln('  Full Sync: ${fullSync['avg'].toStringAsFixed(0)}ms avg (${fullSync['count']} ops)');
      buffer.writeln('    Min: ${fullSync['min']}ms | Max: ${fullSync['max']}ms');
      buffer.writeln('    P90: ${fullSync['p90']}ms | P95: ${fullSync['p95']}ms');
    }
    
    final incrementalSync = getMetricStats('incremental_sync_ms');
    if (incrementalSync['count'] > 0) {
      buffer.writeln('  Incremental: ${incrementalSync['avg'].toStringAsFixed(0)}ms avg (${incrementalSync['count']} ops)');
      buffer.writeln('    Min: ${incrementalSync['min']}ms | Max: ${incrementalSync['max']}ms');
    }
    ...
    if (queryStats['count'] > 0) {
      buffer.writeln('  Average: ${queryStats['avg'].toStringAsFixed(0)}ms (${queryStats['count']} queries)');
      buffer.writeln('  Range: ${queryStats['min']}ms - ${queryStats['max']}ms');
    }
    ...
    if (uploadStats['count'] > 0) {
      buffer.writeln('  Upload: ${uploadStats['avg'].toStringAsFixed(0)}ms avg');
    }
    
    final downloadStats = getMetricStats('download_duration_ms');
    if (downloadStats['count'] > 0) {
      buffer.writeln('  Download: ${downloadStats['avg'].toStringAsFixed(0)}ms avg');
    }
    ...
    if (changesStats['count'] > 0) {
      buffer.writeln('  Avg changes per sync: ${changesStats['avg'].toStringAsFixed(0)}');
    }
